...
#	var inv_stats = $drop_stats_tt/stats_tt_pop_up/stats/compare_container
	...
	var stat_differnces = $drop_stats_tt/stats_tt_pop_up/stats/compare_container.get_children()
	for i in stat_differnces:
		if int(i.text) <= 0:
			i.set("custom_colors/font_color", Color("ff0000"))
		else:
			i.set("custom_colors/font_color", Color("#38ff00"))
